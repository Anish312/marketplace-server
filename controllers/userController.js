...
exports.registerUser = catchAsyncErrors(async (req, res, next) => {
   console.log(req.body)
    ...
    const user =  await OwnerUser.create({
...
exports.loginUser = catchAsyncErrors(async  (req, res, next) => { 
    ...
    const user =  await OwnerUser.findOne({email}).select("+password");
...
exports.getUserDetail = catchAsyncErrors(async (req, res , next) => {
    const user = await OwnerUser.findById(req.user.id);
    if (!user) {
...
exports.updatePassword = catchAsyncErrors(async (req, res ,next) => {
    const user = await OwnerUser.findById(req.user.id).select('+password');
...
exports.updateProfie = catchAsyncErrors(async (req, res ,next) => {
    ...
    const user = await OwnerUser.findByIdAndUpdate(req.user.id , newUserData , {
        new: true,
        reunValidators: true,
...
exports.getAllUser = catchAsyncErrors(async (req, res ,next) => {
    const users = await OwnerUser.find({});
...
exports.getSingleUser = catchAsyncErrors(async (req, res ,next) => {
    ...
    const user = await OwnerUser.findById(req.params.id);
...
exports.updateUserRole = catchAsyncErrors(async (req, res ,next) => {
    ...
    const user = await OwnerUser.findByIdAndUpdate(req.params.id , newuserData , {
        new: true,
        reunValidators: true,
        useFindAndModify: false,
    });
...
exports.deleteUser = catchAsyncErrors(async (req, res ,next) => {
    ...
    const user = await OwnerUser.findById(req.user.id , newuserData , {
        new: true,
        reunValidators: true,
        useFindAndModify: false,
    });
